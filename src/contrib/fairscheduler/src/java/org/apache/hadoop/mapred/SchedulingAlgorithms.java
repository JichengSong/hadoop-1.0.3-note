...
class SchedulingAlgorithms {
  ...
  public static void computeFairShares(
    ...
    double cap = Math.min(totalDemand, totalSlots);//capacity:容量
    double rMax = 1.0;
   //首先计算出weight-2-slot-ratio(权重和slot的比率)，也即rmax,该参数接下来会用于迭代计算FAIR_SHARES
    while (slotsUsedWithWeightToSlotRatio(rMax, schedulables) < cap) {
      rMax *= 2.0;
    }
    // 进行二分查找(最多迭代COMPUTE_FAIR_SHARES_ITERATIONS步,默认25 steps)
    ...
    // 将fair shares的基础设置为我们刚才二分查找出来的值:right
    for (Schedulable sched: schedulables) {
   ...
   * 根据给定的w2sRatio (a wight-to-slog ratio) 计算即将使用的slots数
   */
