...
public class PoolSchedulable extends Schedulable {
  ...
  public void updateDemand() {
    ...
    // 如果需求(demand)超过了这个pool的最大容量，设置demand为最大值max
  ...
  /**
   * Distribute the pool's fair share among its jobs//将pool得到的份额(share)下发给它的jobs
   */
  @Override
...
/**
 * 重写Schedulable类的assignTask方法
 * 如果runningTasks大于为pool配置的maxslots，则不分配,返回null
 */
  @Override
  public Task assignTask(TaskTrackerStatus tts, long currentTime,
    ...
    //根据调度类型(是FIFO还是FAIR)生成comparator
