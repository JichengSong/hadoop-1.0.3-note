...
public class FairScheduler extends TaskScheduler {
  ...
  // How often fair shares are re-calculated //公平调度器重新计算资源的时间间隔
  protected long updateInterval = 500;

  // How often to dump scheduler state to the event log//每间隔多久，将scheduler的状态dump到event log
  protected long dumpInterval = 10000;
  
  // How often tasks are preempted (must be longer than a couple
  // of heartbeats to give task-kill commands a chance to act).多久tasks被抢占(必须大于一次心跳的2倍)
  ...
  // Maximum locality delay when auto-computing locality delays //最大本地延迟？
  private static final long MAX_AUTOCOMPUTED_LOCALITY_DELAY = 15000;
  
  protected PoolManager poolMgr; //PoolManager 管理资源池
  protected LoadManager loadMgr; //负载管理器，管理每个tasktracker的负载，默认使用CapBasedLoadManager
  protected TaskSelector taskSelector;//
  protected WeightAdjuster weightAdjuster; // Can be null for no weight adjuster
  protected Map<JobInProgress, JobInfo> infos = // per-job scheduling variables //每个job调度相关的变量
  ...
  protected boolean assignMultiple; // Simultaneously assign map and reduce? //同时分配map和reduce?
  protected int mapAssignCap = -1;    // Max maps to launch per heartbeat  //默认值是-1
  protected int reduceAssignCap = -1; // Max reduces to launch per heartbeat //默认值是-1
  protected long nodeLocalityDelay;   // Time to wait for node locality //默认是-1 
  protected long rackLocalityDelay;   // Time to wait for rack locality //默认是-1
  protected boolean autoComputeLocalityDelay = false; // Compute locality delay
                                                      // from heartbeat interval
  protected boolean sizeBasedWeight; // Give larger weights to larger jobs //给大作业较大的权重
  protected boolean waitForMapsBeforeLaunchingReduces = true;
  protected boolean preemptionEnabled; //是否允许抢占资源
  protected boolean onlyLogPreemption; // Only log when tasks should be killed//仅当task要被kill掉时打日志
  private Clock clock;
  private JobListener jobListener; //JobListener job侦听器
  ...
  private FairSchedulerEventLog eventLog;//eventLog Event日志
  protected long lastDumpTime;       // Time when we last dumped state to log
  protected long lastHeartbeatTime;  // Time we last ran assignTasks  上一次执行assignTasks的时间
  private long lastPreemptCheckTime; // Time we last ran preemptTasksIfNecessary 上一次判断是否需要抢占的时间.
   ...
   * time delta to update the map and reduce deficits before a new update(). 注意其和org.apache.hadoop.mapred.JobInfo区别
   */
  ...
  public void start() {
    try {
      ...
      boolean logEnabled = conf.getBoolean(//是否开启event日志.
          "mapred.fairscheduler.eventlog.enabled", false);
      if (!mockMode && logEnabled) {//测试模式
      ...
      loadMgr.setTaskTrackerManager(taskTrackerManager);//给loadMgr设置tasktrackerManager,
      loadMgr.setEventLog(eventLog);
      loadMgr.start();     //启动logMgr
      ...
      taskSelector.start();//启动taskSelector. DefaultTaskSelector.start()方法内容为空(do nothing)
      ...
      //如果没有在mapred-site.xml中设置nodeDelay和rackDelay,则autoComputeLocalityDelay=true
      if (defaultDelay == -1 && 
      ...
      // Register servlet with JobTracker's Jetty server//注册scheduler ，可以通过jobtracker_ip:50030/scheduler查看scheduler状态
      if (taskTrackerManager instanceof JobTracker) {
...
/**
  ...
  /**
   * 重写父类TaskScheduler的assignTasks方法,
   */
  @Override
  public synchronized List<Task> assignTasks(TaskTracker tracker)
    ...
    while (true) {
      // 计算循环的结束条件.如果有下列三种情况发生，reject a task type(终止其中一中task type的循环)
  ...
  protected void update() {
    // Making more granular(细粒度的) locking so that clusterStatus can be fetched 
    ...
    // 如果没有在mapred-site.xml中设置nodeDelay和rackDelay,则autoComputeLocalityDelay=true
    // 则nodeDelay,rackDelay默认值为1.5*clusterHeartbeatInterval
    if (autoComputeLocalityDelay) {
      JobTracker jobTracker = (JobTracker) taskTrackerManager;
      //nodeLocalityDelay表示等待利用数据本地性的等待时间。(如果持续了nodeLocalityDelay毫秒仍然没有为相应的split找到能利用数据本地性的tasktracker，则放弃数据本地性)
      nodeLocalityDelay = Math.min(MAX_AUTOCOMPUTED_LOCALITY_DELAY,
          (long) (1.5 * jobTracker.getNextHeartbeatInterval()));
      rackLocalityDelay = nodeLocalityDelay;//机架
    }
    // 
    // Got clusterStatus hence acquiring scheduler lock now.
    synchronized (this) {
      // Reload allocations file if it hasn't been loaded in a while
      poolMgr.reloadAllocsIfNecessary();//默认每10秒钟重新load一次fair-scheduler.xml，因此修改fair-scheduler.xml后会自动生效
  ...
  private void updateRunnability() {
    // Start by marking everything as not runnable;首先将所有job的runnable设置为false
    for (JobInfo info: infos.values()) {
      info.runnable = false;
    }
    // Create a list of sorted jobs in order of start time and priority
    // 创建一个List，存放按照start time和priority排序后的jobs
    ...
    //先将jobs按启动时间和优先级排序，直至达到user limits或pool limits
    Map<String, Integer> userJobs = new HashMap<String, Integer>();//存放user和其running/prepair状态的job数
    Map<String, Integer> poolJobs = new HashMap<String, Integer>();//存放pool和起running/prepair状态的job数
    for (JobInProgress job: jobs) {
      String user = job.getJobConf().getUser();
      String pool = poolMgr.getPoolName(job); //默认pool为default
      ...
      if (userCount < poolMgr.getUserMaxJobs(user) &&
        ...
        if (job.getStatus().getRunState() == JobStatus.RUNNING ||
            job.getStatus().getRunState() == JobStatus.PREP) {//job处于Running或Prepare状态
          userJobs.put(user, userCount + 1); //user的job数+1
          poolJobs.put(pool, poolCount + 1); //pool的job数+1
          JobInfo jobInfo = infos.get(job);
          if (job.getStatus().getRunState() == JobStatus.RUNNING) {
            jobInfo.runnable = true;
          } else {//job处于prep状态，交给jobInitializer去初始化它
