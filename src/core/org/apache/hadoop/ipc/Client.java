...
public class Client {
	...
	private Class<? extends Writable> valueClass;  // IPC call的返回值类型
	private int counter; 						   // 暂时把他理解为Client维护的一个递增计数器,维护counter for call ids
	private AtomicBoolean running = new AtomicBoolean(true); // if client runs
	final private Configuration conf;

	private SocketFactory socketFactory; 		   //socketFacktor是socket工厂,创建Client时需要初始化该属性. how to create sockets
	private int refCount = 1;					   //该Client的Call引用数.
												   //IPC ping报文的发送间隔
	final private static String PING_INTERVAL_NAME = "ipc.ping.interval";
	final static int DEFAULT_PING_INTERVAL = 60000;//默认1分钟发一次IPC ping 
	final static int PING_CALL_ID = -1;			   // IPC ping call的id为-1
	...
	/**********************************************1. ipc.Client.Call 类,描述一个IPC Call(请求/调用)*************************************/
	/** A call waiting for a value. */
	private class Call {
		int id; 			//调用id call id
		Writable param; 	//调用参数 parameter
		Writable value; 	//返回值, null if error
		IOException error;  //异常, exception, null if value
		boolean done;	    //是否执行完成，true when call is done
		/**构造函数，需要传递一个Writable类型的参数,该参数用于描述一次方法调用(Invocation)*/
	...
	/*************************************************2. ipc.Client.Connection类,该线程 读取ipc service发来的response信息并通知调用者.******************/
	/**
	...
	private class Connection extends Thread {
		private InetSocketAddress server;     // ipc server地址. server ip:port
		private String serverPrincipal;       // ipc server的krb5规则名. server's krb5 principal name
		private ConnectionHeader header;      // ipc 连接头. connection header
		private final ConnectionId remoteId;  // 用于标识一个远程连接(包括addresses,protocol,ticket,rpcTimeout, conf)
		private AuthMethod authMethod; // authentication method
		private boolean useSasl;
		private Token<? extends TokenIdentifier> token;
		private SaslRpcClient saslRpcClient;

		private Socket socket = null; // 连接套接字 connected socket
		private DataInputStream in;	  // socket输入流	
		private DataOutputStream out; // socket输出流
		private int rpcTimeout;		  // rpc超时时间	(若该参数大于0，它会重写pingInterval)
		private int maxIdleTime; 	  // 最大空闲时间,若Connection空闲时间超过maxIdleTime将会被剔除
		...
		public Connection(ConnectionId remoteId) throws IOException {
			...
			//为该线程命名,设置为守护线程
			this.setName("IPC Client ("
					+ socketFactory.hashCode()
					+ ") connection to "
					+ remoteId.getAddress().toString()
					+ " from "
					+ ((ticket == null) ? "an unknown user" : ticket
							.getUserName()));
			this.setDaemon(true);
		}
		...
		/*******************BEGIN**************ipc.Client.Connection.PingInputStream类*********************************************************************/
		/**PingInputStream.read()读取数据超时时，会向远程pic server发一个ipc ping call.如果ping ok,它会持续下去，直到读取到至少1字节数据.
		...
		/*******************END**************ipc.Client.Connection.PingInputStream类*********************************************************************/
		...
		private synchronized boolean shouldAuthenticateOverKrb()
			...
			// Make sure user logged in using Kerberos either keytab or TGT
					...
					(loginUser.equals(currentUser) || loginUser
							.equals(realUser))) {
				return true;
			}
			return false;
		...
		/**如果Server address对应的host name发生了改变，即使更新server address
			...
			while (true) {
				try {//1.创建socket
					...
					if (UserGroupInformation.isSecurityEnabled()) {
						KerberosInfo krbInfo = remoteId.getProtocol()
								.getAnnotation(KerberosInfo.class);
						if (krbInfo != null
								&& krbInfo.clientPrincipal() != null) {
							String host = SecurityUtil
									.getHostFromPrincipal(remoteId.getTicket()
											.getUserName());

							// If host name is a valid local address then bind
							// socket to it
							InetAddress localAddr = NetUtils
									.getLocalInetAddress(host);
							if (localAddr != null) {
		...
		/**
		 * Three failures are handled here - 1) SocketTimeout failures. We just
		 * retry after sleeping for sometime 2) Kerberos replay attack failure.
		 * If multiple clients with the same principal try to connect to the
		 * same server at the same time, the server assumes a replay attack is
		 * in progress. This is a feature of kerberos. In order to work around
		 * this, what is done is that the client backs off randomly and tries to
		 * initiate the connection again. 3) The third problem is to do with
		 * ticket expiry. To handle that, a relogin is attempted. Failure
		 * scenarios: (1) Client authenticates over kerberos and the connection
		 * fails due to kerberos replay attack, or his ticket has expired or
		 * does not exist. Also connection can fail due to timeout. (2) Client
		 * authenticates over DIGEST and the connection fails due to timeout.
		 * For MapReduce tasks, assuming the token is valid, only (2) can
		 * happen.
		 */
		private synchronized void handleSaslConnectionFailure(
				final int currRetries, final int maxRetries,
				final Exception ex, final Random rand,
			...
			ugi.doAs(new PrivilegedExceptionAction<Object>() {
				public Object run() throws IOException, InterruptedException {
					...
					closeConnection();
					disposeSasl();
					// TODO: currRetries is overloaded here.For both kerberos
					// and timeout
					// failures, currRetries is used to maintain the number of
					// failures
					// seen so far. Should fix this to distinguish between the
					// two.
					// TODO: Refactor the method to remove duplicated code
					// (e.g.,the logic
					// inside "currRetries < maxRetries" could be factored out
					// to apply
					...
					if (ex instanceof SocketTimeoutException) {
						if (currRetries < maxRetries) {
							LOG.warn("Encountered "
									...
									+ serverPrincipal);
							// we are sleeping with the Connection lock held but
							// since this
							// connection instance is being used for connecting
							// to the server
		...
		private synchronized void setupIOstreams() throws InterruptedException {
			...
			try {
				...
				Random rand = null;
				while (true) {
					setupConnection();		  //@@@@ 1.建立socket连接@@@@
					InputStream inStream = NetUtils.getInputStream(socket);
					OutputStream outStream = NetUtils.getOutputStream(socket);
					writeRpcHeader(outStream);//@@@@ 2.写rpc头信息 @@@@ 0.Client和Server建立连接后，首先会写RPC头信息
					if (useSasl) {			  
							...
							continueSasl = ticket
									.doAs(new PrivilegedExceptionAction<Boolean>() {
										...
										public Boolean run() throws IOException {
											return setupSaslConnection(in2,
													out2);
										}
									});
						} catch (Exception ex) {
						...
						if (continueSasl) {
							...
							outStream = saslRpcClient
									.getOutputStream(outStream);
						} else {
							// fall back to simple auth because server told us
							// so.
							authMethod = AuthMethod.SIMPLE;
					...
					this.out = new DataOutputStream(new BufferedOutputStream(
							outStream));
					writeHeader(); //@@@@ 3.写connection头信息:protocol authorized ugi@@@@
		...
		private void writeRpcHeader(OutputStream outStream) throws IOException {
			...
			out.write(Server.HEADER.array()); //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ RPC 头信息: "hrpc" 4字节,version 1字节，认证方式1字节 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		 ...
		 */
		public void sendParam(Call call) {//发送一次调用call.
			if (shouldCloseConnection.get()) {
				return;
			}
	...
	public Writable call(Writable param, InetSocketAddress addr,
			UserGroupInformation ticket) throws InterruptedException,
			IOException {
	...
	public Writable call(Writable param, ConnectionId remoteId)
			throws InterruptedException, IOException {
		Call call = new Call(param);						  //1.构建Call对象
		Connection connection = getConnection(remoteId, call);//2.获取/建立连接.(首次建立连接后,会立即向Server发送RPC Header和Connection Header)
		connection.sendParam(call); 						  //3.发送param数据:(dataLength 4字节,call.id 4字节,call.param 也即RPC.Invocation对象字节流)
		boolean interrupted = false;
		synchronized (call) {
			while (!call.done) {							  //4.等待直至该call的状态为完成(call.done=true)	
				try {
			...
			for (int i = 0; i < params.length; i++) {
				...
				try {
					ConnectionId remoteId = ConnectionId.getConnectionId(
							addresses[i], protocol, ticket, 0, conf);
					Connection connection = getConnection(remoteId, call);
					connection.sendParam(call); // send each parameter
				} catch (IOException e) {
					// log errors
					LOG.info(
							"Calling " + addresses[i] + " caught: "
									+ e.getMessage(), e);
	...
	/**从connections池中获取一个连接/或创建一个连接. 将call添加到该连接等待被处理.
	...
	/****************************************5.ipc.Client.ConnectionId类**************************************************/
	/**这个类持有address和user ticket. client到servers的连接由remoteAddress,protocol,ticket唯一标识.
	...
	static class ConnectionId {
		...
		ConnectionId(InetSocketAddress address, Class<?> protocol,
				...
				String serverPrincipal, int maxIdleTime, int maxRetries,
				boolean tcpNoDelay, int pingInterval) {
		...
		private static String getRemotePrincipal(Configuration conf,
				InetSocketAddress address, Class<?> protocol)
				throws IOException {
