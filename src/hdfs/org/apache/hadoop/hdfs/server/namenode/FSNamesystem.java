 ...
 */
  ...
  FSNamesystem(FSImage fsImage, Configuration conf) throws IOException {
    setConfigurationParameters(conf);
    this.dir = new FSDirectory(fsImage, this, conf);//初始化dir
    dtSecretManager = createDelegationTokenSecretManager(conf);
  }
  ...
  void startFile(String src, PermissionStatus permissions,
                ...
                ) throws IOException {//调用startFileInternal,这里将append设置为false了
  ...
  private synchronized void startFileInternal(String src,
    ...
    /**create:****************************************************/
    try {//(1).若src对应的inode存在且处于underConstruction状态，则收inode上的租约//force=false，表示不强制收回.
      ...
      if (append) {//(3).如果是append操作,且myFile不存在或myFile是目录，则抛出异常
        if (myFile == null) {
        ...
        }//(4)如果非append，但src已经存在，且overwrite为true,则删除当前文件(目录)
      ...
      //(5).获取一个clientMachine对应的DatanodeDescriptor，如果clientMachine不是datanode机器，则clientNode=null
      DatanodeDescriptor clientNode = 
        host2DataNodeMap.getDatanodeByHost(clientMachine);
      /*******************************这一部分很重要*************************/
      if (append) {//(6).属于append操作
        //
      ...
      } else {//(7).不是append操作 (注:调用fs.create()时，append的值为false)
       // 
       //Now we can add the name to the filesystem. This file has no blocks associated with it. 
       //(7.1).检查inodes数目是否超过最大值
        checkFsObjectLimit();
        //(7.2)获取全局唯一的genstamp，用于标记本次操作
        long genstamp = nextGenerationStamp();
        //(7.3)在FSDirectory(也即dir实例)增加一个file
        INodeFileUnderConstruction newNode = dir.addFile(src, permissions,
            replication, blockSize, holder, clientMachine, clientNode, genstamp);
        if (newNode == null) {
          throw new IOException("DIR* NameSystem.startFile: " +
                                "Unable to add file to namespace.");
        }//(7.4)为clientName添加该文件的租约
  ...
  /**
   * Recover lease;//收回租约:撤销当前lease holder的lease,并开始收回租约,从而使文件可以被强制关闭(closed)
  ...
  throws IOException {
    if (isInSafeMode()) {//如果当前hdfs处于安全模式，则无法收回租约，抛出异常.
      throw new SafeModeException(
          "Cannot recover the lease of " + src, safeMode);
    }
    if (!DFSUtil.isValidName(src)) {//文件名是否有效
      throw new IOException("Invalid file name: " + src);
    }

    INode inode = dir.getFileINode(src);
    if (inode == null) {//当前文件不存在，抛出异常
      throw new FileNotFoundException("File not found " + src);
    }

    if (!inode.isUnderConstruction()) {//当前文件并未处于UnserConstruction状态，直接返回true
      return true;
    }
    if (isPermissionEnabled) {//是否允许写
      checkPathAccess(src, FsAction.WRITE);
    }
    //回收租约(force=true 强制回收)
    recoverLeaseInternal(inode, src, holder, clientMachine, true);
    return false;
  }
  /**如果fileInode存在,且处于UnderConstruction()状态，则回收该inode上的租约;force表示是否强制收回.只有client.recoverLease(Path)时，force才是true*/
  ...
  throws IOException {
    if (fileInode != null && fileInode.isUnderConstruction()) {
      ...
      //
      // 如果文件处于unserConstruction状态，则它一定在leases中；找到对应的租约:
      // 
      // *！！注意，这个lease是当前client所对应的lease
      Lease lease = leaseManager.getLease(holder);
      //
      //(1).我们找到了该file的lease,但发现orginal holder正在尝试recreate这个文件。这种情况是不应该发生的.则抛出异常.
      // We found the lease for this file. And surprisingly the original holder is trying to recreate this file. This should never occur.
      // 
      if (!force && lease != null) {
        Lease leaseFile = leaseManager.getLeaseByPath(src);
        if (leaseFile != null && leaseFile.equals(lease)) {//如果src上的lease就是client持有的lease(也即当前client就是orgin client),且force=false(表明client没有主动调用recoverLease(path)),
          throw new AlreadyBeingCreatedException(          //表明orgin client在recreate这个src,所以要抛出异常
      ...
      //(2).找到该inode上的lease
      // *！！注意，这个lease是inode上的lease；是最初的lease. 也即使该inode变成underConstruction状态的client所持有的lease.  //Find the original holder.
      lease = leaseManager.getLease(pendingFile.clientName);
      //(3).如果该inode上的lease是null,则抛出异常.(因为不可能出现inode处于UnderConstruction，但lease为null的情况)
      if (lease == null) {
        throw new AlreadyBeingCreatedException(
                    "failed to create file " + src + " for " + holder +
                    " on client " + clientMachine + 
                    " because pendingCreates is non-null but no leases found.");
      }//(4).如果该inode上的lease非null,由于当前client并不是orgin client(也既inode lease的client),则有两中情况:
      if (force) {//(4.1).force=true:当前client显示调用fs.recoverLease(src)
      ...
      } else {//(4.2).force=false,当前client要写(create/append)文件src,该文件正在被另一个client写，所以要抛出异常.
        //
   ...
   * recovered.// 当lease过期时该方法会被调用；一但lease过期，该lease对应的dfsclient写的所有文件都需要进行恢复"recovered".
   */
  ...
  void internalReleaseLeaseOne(Lease lease, String src) throws IOException {
    ...
    // 为pendingFile初始化租约回收(lease recovery). 如果没有blocks和这个file相关，则立即收回租约；否则，renew租约，并触发lease recovery
    ...
    if (pendingFile.getTargets() == null ||//getTargets()会获取文件最后一个block的位置.如果targets为空或长度为0
        pendingFile.getTargets().length == 0) {
      if (pendingFile.getBlocks().length == 0) {//(1)且文件的block数也为0：
        finalizeINodeFileUnderConstruction(src, pendingFile);
        NameNode.stateChangeLog.warn("BLOCK*"
          + " internalReleaseLease: No blocks found, lease removed for " +  src);
        return;
      }
      // (2) 但文件的block数不为0,则为根据blocksMap为该文件构建targets
    ...
    // start lease recovery of the last block for this file. 为该文件的最后一个 block做lease recovery
    pendingFile.assignPrimaryDatanode();//pendingFile的最后一个block将被添加到datanodeDescriptor.recoverBlocks中，其他datanode从该datanode恢复该block
    Lease reassignedLease = reassignLease(//重新分配租约, 租约的holder变成了"NN_Recovery",他是
  ...
  private Lease reassignLease(Lease lease, String src, String newHolder,
    ...
    if(newHolder == null)//如果newHolder是null,相当于没有重新分配租约，返回原来的lease
      return lease;
