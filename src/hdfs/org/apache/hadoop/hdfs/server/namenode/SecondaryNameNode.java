...
public class SecondaryNameNode implements Runnable {
  ...
  private String fsName;//这个值是namenode配置的dfs.http.address指定的，默认是namenodeIp:50070
  ...
  private void initialize(final Configuration conf) throws IOException {
    ...
    // 默认情况下,每一个小时做一次checkpoint，如果edit log达到4M，也会触发checkpoint
  ...
  public void doWork() {
    ...
    /**每5分钟c从namenode poll下来edit log，查看它的大小*/
    // Poll the Namenode (once every 5 minutes) to find the size of the
    // pending edit log.
    //
    long period = 5 * 60;              // 5 minutes
    long lastCheckpointTime = 0;	   // 离上一次checkpoint的时间
    if (checkpointPeriod < period) {   // 如果用户设置的checkpointPeriod小于5分钟，则checkPointPeriod为5分钟
      period = checkpointPeriod;
    }
    /**shuoldRun在SecondaryNamenode启动后为true,shutdown时为false。SNN启动后将一直执行该while循环*/
    while (shouldRun) {
      try {
        Thread.sleep(1000 * period);	//sleep 5分钟
      ...
      try {
        ...
        /**如果namenode的editLogSize大于checkpointSize，或者lastCheckpointTime大于checkpointPeriod,则做一次checkpoint*/
  ...
  /**SNN下载和上传fsimage都是通过TransferFsImage.getFileClient(fsName,fileid,file[])方法；
   * 上传时，fileid以"putimage="开头，下载时，fileid以"getimage=”开头
  ...
  void doCheckpoint() throws IOException {
	//(1).先做一些初始化工作.(准备checkpoint所需的本地目录)
    // Do the required initialization of the merge work area.
    startCheckpoint();
    //(2).通知namenode开始做checkpoint，namenode会将ckptstat设置为ROLLED_EDITST,将edit文件关闭，
    //   事务日志写到edit.new. 返回的sig签名供SNN上传fsimage时使用.
    // tell the namenode to start logging transactions in a new edit file.Retuns a token that would be used to upload the merged image.
    ...
    //(3).下载fsimage和edits,并合并.
    downloadCheckpointFiles(sig);   // Fetch fsimage and edits
    doMerge(sig);                   // Do the merge
  
    //(4).将合并后的fsimage上传给namenode
    ...
    //(5).通知namenode将fsimage.ckpt更改为fsimage,edits.new更改为edits，ckptState从UPLOAD_DONE更改为START
  ...
  private void startCheckpoint() throws IOException {
    checkpointImage.unlockAll(); //1.解锁
    checkpointImage.getEditLog().close();//2.关闭edit log
    checkpointImage.recoverCreate(checkpointDirs, checkpointEditsDirs);//3.创建checkpoint目录
    checkpointImage.startCheckpoint();//4.为checkpoint准备目录.(清理checkpoint目录下的文件)
  }
