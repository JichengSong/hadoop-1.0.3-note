...
/**SNN通过TransferFsImage方法上传和下载fsimage. 这个类用于响应SNN发来的http请求.
    ...
    try {
      ...
      UserGroupInformation.getCurrentUser().doAs(new PrivilegedExceptionAction<Void>() {
        ...
        public Void run() throws Exception {
          if (ff.getImage()) {
            // (1). 发送fsImage //send fsImage
            TransferFsImage.getFileServer(response.getOutputStream(),
                                          nnImage.getFsImageName()); 
          } else if (ff.getEdit()) {
            // (2). 发送edits  //send edits
            TransferFsImage.getFileServer(response.getOutputStream(),
                                          nnImage.getFsEditName());
          } else if (ff.putImage()) {
            // (3.1). 判断ckptState上一个状态是否为ROLL_EDITS,若是,则将状态更改为UPLOAD_START,并下载合并后的fsimage
            ...
            //(3.2) 更改namenode fsimage的ckptstate值为UPLOAD_DONE
