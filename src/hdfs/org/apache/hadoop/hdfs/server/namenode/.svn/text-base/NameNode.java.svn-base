...
public class NameNode implements ClientProtocol, DatanodeProtocol,
  ...
  /**NameNode的两类log:Log和ChangeLog*/
  public static final Log LOG = LogFactory.getLog(NameNode.class.getName());
  public static final Log stateChangeLog = LogFactory.getLog("org.apache.hadoop.hdfs.StateChange");
  public FSNamesystem namesystem; // TODO: This should private. Use getNamesystem() instead. 一个NameNode有一个FSNamesystem实例
  /** RPC server */
  ...
  */
  ...
  private Thread emptier;//清理线程，用于清理各个user的trash
  ...
  private void initialize(Configuration conf) throws IOException {
    InetSocketAddress socAddr = NameNode.getAddress(conf);//如果配置了kerberos验证，则进行验证
    UserGroupInformation.setConfiguration(conf);
    SecurityUtil.login(conf, DFSConfigKeys.DFS_NAMENODE_KEYTAB_FILE_KEY, 
        DFSConfigKeys.DFS_NAMENODE_USER_NAME_KEY, socAddr.getHostName());
    int handlerCount = conf.getInt("dfs.namenode.handler.count", 10);//namenode的rpc hander数,默认10
    
    //设置服务级别的安全认证  set service-level authorization security policy 
    ...
    myMetrics = NameNodeInstrumentation.create(conf);//初始化myMetrics
    this.namesystem = new FSNamesystem(this, conf);//创建FSNamesystem. 关键内容在这一步. 

    if (UserGroupInformation.isSecurityEnabled()) {
      namesystem.activateSecretManager();
    }

    //如果配置了"dfs.namenode.servicerpc-address",则创建RPC Server //create rpc server
    ...
    }//根据"fs.default.name" 创建NameNode Server
    ...
    startHttpServer(conf);//开启web server
    this.server.start();  //开启NameNode的RPC Server  //start RPC server   
    if (serviceRpcServer != null) {//如果配置了serviceRpcServer，启动它
      serviceRpcServer.start();      
    }
    startTrashEmptier(conf);//开启trash清理线程.
  }
  ...
  /**************************从这里开始实现NamenodeProtocol协议定义的接口****************************/
  /*********************************************************************************************/
  // NamenodeProtocol
  /*********************************************************************************************/
  /**
  ...
  public void create(String src, 
    ...
    }//调用FSNamesystem的startFile方法创建文件,其中holder的值是null
  ...
  /**HDFS-630修复如下bug:client每次申请dd时,NN给出的dd在client的黑名单excludedNodes里，client re-request dd时NN可能把同一个dd发给自己.
  ...
  public LocatedBlock addBlock(String src,
    ...
    stateChangeLog.debug("*BLOCK* NameNode.addBlock: file "
                         +src+" for "+clientName);
    //调用FSNamesystem的getAdditionalBlock方法，为src文件申请一个新的block
  ...
  /**client写完一个file时，close这个file。如果complete返回true，表示文件被close.
  /** {@inheritDoc} */
  ...
  /**删除文件,recursive表示是否递归删除子目录(对于directory文件而言).
  /** {@inheritDoc} */
  ...
  /**检查文件长度是否超过最大值(8000)及文件目录深度是否超过1000.
  ...
  /**获取目录的局部列表a partial listing starting after startAfter*/
  @Override
  ...
  /**获取集群的datanodes状态信息，比如type为live,表示获取所有活着的datanode
   */
  ...
  /**滚动edit log: 关闭edit log,创建edits.new文件,将ckptState设为ROLL_EDITS
   * Roll the edit log.
   */
  ...
  /**滚动fsimage:将fsImage.ckpt 更名为fsImage,edits.new更名为edits. 将ckptState从UPLOAD_DONE更改为START
   * Roll the image 
   */
