...
public class LeaseManager {
  ...
  synchronized Lease reassignLease(Lease lease, String src, String newHolder) {
    ...
    if (lease != null) {//删除旧lease
      removeLease(lease, src);
    }
    return addLease(newHolder, src);//返回新lease
  }
  ...
  /************************************************************
   * A Lease governs all the locks held by a single client.//一个lease管理但个client持有的所有锁.
   * For each client there's a corresponding lease, whose//每个client都有对应的lease
   * timestamp is updated when the client periodically
   * checks in.  If the client dies and allows its lease to//如果client挂掉，且允许lease过期，则相应的锁会被释放.
  ...
  class Lease implements Comparable<Lease> {
    ...
    public boolean expiredHardLimit() {//如果硬期限过了，返回true
      return FSNamesystem.now() - lastUpdate > hardLimit;
    }

    /** @return true if the Soft Limit Timer has expired */
    public boolean expiredSoftLimit() {//如果软期限过了,返回true
      return FSNamesystem.now() - lastUpdate > softLimit;
    }

    /**
     * @return the path associated with the pendingFile and null if not found.//返回pendingFile相关的path
     */
  ...
  synchronized void checkLeases() {
    for(; sortedLeases.size() > 0; ) {
      ...
      if (!oldest.expiredHardLimit()) {//0.如果最旧的lease还没过期，则退出.
        return;
      }

      LOG.info("Lease " + oldest + " has expired hard limit");
      //1.removing用来存放该lease管理的path;
      ...
      for(String p : leasePaths) {//2.租约过期了，需要善后lease管理的文件.(being written 状态的文件需要被关闭)
        try {
      ...
      //3.将removing中的所有path从lease.paths中删除,然后将lease从leases中删除(leases.remove(lease.holder))
